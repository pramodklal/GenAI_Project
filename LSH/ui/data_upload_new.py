...
# Page header
st.title("📤 Data Import - Healthcare Digital")
st.markdown("Upload CSV files and manage collections in Astra DB")
...
with tab1:
    ...
    # Define standard collections for Healthcare Digital system
    standard_collections = {
        "patients": "Patient master data",
        "patient_dietary_profiles": "Nutritional targets and restrictions",
        "meal_orders": "Patient meal orders",
        "evs_staff": "EVS staff profiles",
        "agent_activities": "Agent execution logs",
        "system_audit_logs": "HIPAA audit trail",
        "food_inventory": "Ingredient inventory",
        "production_schedules": "Meal production plans",
        "menu_items": "Menu catalog (Vector-enabled)",
        "evs_tasks": "EVS tasks (Vector-enabled)"
    }
    ...
    with col1:
        ...
        selected_option = st.selectbox(
            ...
            help="Choose a collection from the Healthcare Digital system"
        )
    ...
    if not collection_exists:
        ...
        vector_collections = ["menu_items", "evs_tasks"]
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("🚀 Start Import", type="primary", use_container_width=True):
                    ...
                    # Show errors if any
                    if error_rows and not skip_errors:
